...
grep 'nfs_test" 10.234.234.234' /etc/exports | report_result

echo 'Checking NFS mount on VM.'
vagrant ssh -c'mount'  -- -q | grep nfs_test | report_result
...
vagrant ssh -c'[ -f /tmp/nfs_test/local.txt ]'  -- -q | report_result

echo 'Checking local test file contents on VM.'
vagrant ssh -c'cat /tmp/nfs_test/local.txt'  -- -q | grep ^test1234 | report_result

echo 'Checking local test file ownership on VM.'
vagrant ssh -c'ls -la /tmp/nfs_test/local.txt' -- -q | grep "test_user www-data" | report_result

echo 'Creating remote test file.'
vagrant ssh -c'echo test4321 > /tmp/nfs_test/remote.txt' -- -q | report_result
